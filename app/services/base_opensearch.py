...
class BaseOpenSearchService:
    ...
    async def create_index(self):
        if not self.client.indices.exists(index=self.index_name):
            index_body = {
                ...
                "mappings": {"properties": self.mappings},
            }
    ...
    async def update_document(self, id: str, document: dict):
        return self.client.update(
            index=self.index_name, id=id, body={"doc": document}, refresh=True
        )

    async def delete_document(self, id: str):
        return self.client.delete(index=self.index_name, id=id, refresh=True)
